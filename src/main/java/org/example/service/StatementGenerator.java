...
public class StatementGenerator {
    ...
    public void generate(Account account, TransactionManager transactionManager) {
        printHeader(account);
        Transaction[] relevantTxns = filterTransactions(account, transactionManager);
        int count = countNonNull(relevantTxns);
     ...
     * @return array of matching transactions (may contain trailing nulls)
     */
    private Transaction[] filterTransactions(Account account, TransactionManager transactionManager) {
        int total = transactionManager.getTransactionCount();
        Transaction[] all    = transactionManager.getTransactions();
        Transaction[] result = new Transaction[total];
        int idx = 0;
        for (int i = 0; i < total; i++) {
            if (all[i].getAccountNumber().equals(account.getAccountNumber())) {
                result[idx++] = all[i];
            }
        }
        return result;
     ...
     */
    private void printTransactions(Transaction[] txns, int count) {
    ...
    /**
     * Counts non-null elements starting from index 0 in {@code arr}.
     *
     * @param arr array that may have trailing nulls
